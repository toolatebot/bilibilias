...
object FileUtils {
    ...
    fun getFileNameNoExtension(filePath: String): String? {
        return if (filePath.isEmpty()) {
            ...
            val lastPoi = filePath.lastIndexOf(46.toChar())
            val lastSep = filePath.lastIndexOf(File.separator)
            if (lastSep == -1) {
                if (lastPoi == -1) filePath else filePath.substring(0, lastPoi)
            } else {
                if (lastPoi != -1 && lastSep <= lastPoi) filePath.substring(
                    lastSep + 1,
                    lastPoi
                ) else filePath.substring(lastSep + 1)
            }
        }
    }
