package com.imcys.bilibilias.home.ui.activity.video.model
...
import com.imcys.bilibilias.home.ui.activity.video.PlayVideoActivity
 ...
 * 解析视频的ViewModel
 */
class DkVideoViewModel(
    val context: Context, private val asVideoBinding: ActivityDkVideoBinding,
) : ViewModel() {
    ...
    fun downloadVideo(
        ...
        HttpUtils.addHeader("cookie", BaseApplication.cookies)
            ...
            .get("${BilibiliApi.videoPlayPath}?bvid=${(context as PlayVideoActivity).bvid}&cid=${context.cid}&qn=64&fnval=4048&fourk=1",
    ...
    fun downloadDanMu(videoBaseBean: VideoBaseBean) {
        GlobalScope.launch(Dispatchers.IO) {
            ...
            val response =
                HttpUtils.asyncGet("${BilibiliApi.videoDanMuPath}?oid=${(context as PlayVideoActivity).cid}")
    ...
    fun saveDanmaku(bytes: ByteArray, cid: Int) {
        ...
        val decompressBytes =
            (context as PlayVideoActivity).decompress(bytes) //调用解压函数进行解压，返回包含解压后数据的byte数组
